# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.1.3] - 2026-06-15

- Implement service ID support and enhance consent handling for Borlabs Cookie integration

## [1.1.2] - 2026-06-15

- Enhance vendor storage and script loading functionality

## [1.1.1] - 2026-06-15

- Enhance uninstall process and update asset handling

## [1.1.0] - 2026-06-15

- Implement Real Cookie Banner integration for mave player embeds
- Add Borlabs Cookie integration for mave player embeds

## [1.0.4] - 2026-06-15

- Enable GitHub auto updates by default and restore settings page section for managing update preferences.

## [1.0.3] - 2026-06-15

- Update plugin name formatting to "WE Mave Video" across various files for consistency and improved localization support.
- Refactor code style in we-mave-video.php for consistency. Updated plugin name formatting and improved spacing in conditional statements and function calls.

## [1.0.2] - 2026-06-15

- Update .gitignore to include .vscode directory and modify wp-cli.phar path in scripts. Adjusted update-pot.js to exclude unnecessary directories.

## [1.0.1] - 2026-06-15

- Enhance plugin functionality and localization support. Updated .gitignore to include node_modules and various log files. Modified readme.txt to reference CHANGELOG.md. Updated plugin URI and added GitHub update support in options. Improved block editor translations and added localization for player settings in JavaScript. Introduced GitHub auto-update option in settings page.
- init

## [1.0.0] - 2026-06-10

### Added

- Self-hosted `@maveio/components` player bundle with themes and locale dependencies
- Shortcode `[we_mave_player embed="…"]` with global defaults and per-embed overrides
- Optional Gutenberg block and Enfold-friendly snippet generator
- Admin settings for component update schedule, CDN debug mode, and player defaults
- GitHub release workflow, changelog tooling, and optional plugin self-updates
- Translation tooling (POT/JSON generation via WP-CLI)

[1.0.0]: https://github.com/gbyat/we-mave-video/releases/tag/v1.0.0
[1.0.1]: https://github.com/gbyat/we-mave-video/releases/tag/v1.0.1

[1.0.2]: https://github.com/gbyat/we-mave-video/releases/tag/v1.0.2

[1.0.3]: https://github.com/gbyat/we-mave-video/releases/tag/v1.0.3

[1.0.4]: https://github.com/gbyat/we-mave-video/releases/tag/v1.0.4

[1.1.0]: https://github.com/gbyat/we-mave-video/releases/tag/v1.1.0

[1.1.1]: https://github.com/gbyat/we-mave-video/releases/tag/v1.1.1

[1.1.2]: https://github.com/gbyat/we-mave-video/releases/tag/v1.1.2

[1.1.3]: https://github.com/gbyat/we-mave-video/releases/tag/v1.1.3
