<?php
/**
 * GitHub updater for plugin self-updates.
 *
...
/**
 * Handles update checks against the latest GitHub release.
 */
final class Plugin_Updater {
	...
	private function is_updates_enabled(): bool {
		...
		return isset( $settings['github_updates_enabled'] )
			&& 'yes' === sanitize_key( (string) $settings['github_updates_enabled'] );
	}
	...
	public function get_github_response(): void {
		if ( ! $this->is_updates_enabled() || ! defined( 'WE_MAVE_VIDEO_GITHUB_REPO' ) ) {
			return;
		}

		$response = wp_remote_get(
			'https://api.github.com/repos/' . WE_MAVE_VIDEO_GITHUB_REPO . '/releases/latest',
			array(
				'timeout' => 30,
				'headers' => array(
	...
	public function modify_transient( $transient ) {
		...
		$tested       = is_array( $compat ) && isset( $compat['Tested up to'] ) ? (string) $compat['Tested up to'] : (string) ( $this->plugin['Tested up to'] ?? '6.8' );
		$requires     = is_array( $compat ) && isset( $compat['Requires at least'] ) ? (string) $compat['Requires at least'] : (string) ( $this->plugin['Requires at least'] ?? '6.5' );
		$requires_php = is_array( $compat ) && isset( $compat['Requires PHP'] ) ? (string) $compat['Requires PHP'] : (string) ( $this->plugin['Requires PHP'] ?? '8.0' );
		...
		$transient->response[ $this->basename ] = (object) array(
			...
			'url'          => 'https://github.com/' . WE_MAVE_VIDEO_GITHUB_REPO,
	...
	public function plugin_popup( $result, string $action, $args ) {
		...
		$changelog = isset( $this->github_response->body ) && is_string( $this->github_response->body )
			? $this->github_response->body
			: __( 'No changelog available.', 'we-mave-video' );
		...
		return (object) array(
			...
			'homepage'          => 'https://github.com/' . WE_MAVE_VIDEO_GITHUB_REPO,
	...
	private function resolve_release_asset_url(): string {
		if ( ! $this->github_response || ! defined( 'WE_MAVE_VIDEO_GITHUB_REPO' ) ) {
			return '';
		}
		...
		if ( isset( $this->github_response->assets ) && is_array( $this->github_response->assets ) ) {
			foreach ( $this->github_response->assets as $asset ) {
				...
				if ( 'we-mave-video.zip' !== $name ) {
					continue;
				}
				if ( ! preg_match( '#^https://github\.com/' . preg_quote( WE_MAVE_VIDEO_GITHUB_REPO, '#' ) . '/releases/download/' . preg_quote( $tag, '#' ) . '/we-mave-video\.zip$#', $url ) ) {
					continue;
				}
				return $url;
			}
	...
	private function get_plugin_data_from_zip( string $zip_url ) {
		...
		if ( ! preg_match( '#^https://github\.com/' . preg_quote( WE_MAVE_VIDEO_GITHUB_REPO, '#' ) . '/releases/download/v?\d+\.\d+\.\d+/we-mave-video\.zip$#', $zip_url ) ) {
			return false;
		}

		$cache_key = 'wmv_github_release_zip_' . md5( $zip_url );
		...
		for ( $i = 0; $i < $zip->numFiles; $i++ ) {
			...
			if ( is_string( $name ) && preg_match( '#^we-mave-video/we-mave-video\.php$#', $name ) ) {
				$plugin_file = $name;
				break;
			}
		}
